...
(function () {
  ...
  // Local playback advances via the <audio> element's own "ended" event
  // (see above). Casting plays entirely on the receiver device, so we
  // need this separate signal from cast.js to know when to advance.
  TimberlineCast.onRemoteEnded(() => {
    if (isCasting) next();
  });
